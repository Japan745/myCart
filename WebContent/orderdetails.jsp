...
<%
	...
	<table id = "customers" width="100%">
		<tr>
			...
			<th><%out.println(request.getParameter("date"));%></th>
			<th><h2>Order id : </h2></th>
			<th><% out.println(request.getParameter("id"));%></th>
		</tr>
		<tr>
			<th>Product </th>
			<th>Quantity</th>
		...
		<% while(rs.next()){ id = rs.getString("order_id"); %>
		<tr>
			<td><%out.println(rs.getString("product_name"));%></td>
			<td><%out.println(rs.getString("quantity"));%></td>
			<!-- <td><%out.println(rs.getString("price"));%></td>-->
			<td><%if(rs.getInt("order_status")==1){out.print("Completed");}else{out.println("Pending");} %></td>
		</tr>
		<%} %>
		<tr>
			<!--  <td> Back : </td>-->
			<td><a href="orderlist.jsp"><button>Back</button></a></td>
			<td>Click to complete : </td>
			<td><a href="ordercomplete.jsp?id=<%=id%>"><button name="complete">complete</button></a></td>
		</tr>
	</table>
